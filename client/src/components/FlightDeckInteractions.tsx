...
const INITIAL: Record<PanelKey, Position> = {
  altitude: { x: 28, y: 118 },
  coordinates: { x: 28, y: 224 },
  status: { x: 28, y: 330 },
};
...
export default function FlightDeckInteractions() {
  ...
  useEffect(() => {
    ...
    const move = (event: MouseEvent) => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        const speed = Math.min(1.8, Math.hypot(vx, vy) / 22);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      cards.forEach((card) => { card.removeEventListener("mousemove", onCardMove); card.removeEventListener("mouseleave", resetCard); });
    };
  }, []);

  const altitude = Math.round((1 - pointer.y / Math.max(window.innerHeight, 1)) * 100);
  ...
  return <>
    <div className="telemetry-layer" aria-label="Interactive telemetry panels">
      <div className={`telemetry-widget telemetry-draggable ${dragging === "altitude" ? "dragging" : ""}`} data-telemetry="altitude" style={panelStyle("altitude")} onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); setDragging("altitude"); }} role="group" aria-label="Draggable relative altitude panel">
        <span className="drag-handle">DRAG</span><small>RELATIVE ALTITUDE</small><strong>{altitude.toString().padStart(3, "0")} <i>UI</i></strong><em>POINTER Y / LIVE</em>
      </div>
      <div className={`telemetry-widget telemetry-draggable ${dragging === "coordinates" ? "dragging" : ""}`} data-telemetry="coordinates" style={panelStyle("coordinates")} onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); setDragging("coordinates"); }} role="group" aria-label="Draggable pointer coordinates panel">
        <span className="drag-handle">DRAG</span><small>SCREEN COORDINATES</small><strong>{xPercent.toString().padStart(3, "0")} : {yPercent.toString().padStart(3, "0")}</strong><em>LIVE POINTER POSITION</em>
      </div>
      <div className={`telemetry-widget telemetry-draggable ${dragging === "status" ? "dragging" : ""}`} data-telemetry="status" style={panelStyle("status")} onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); setDragging("status"); }} role="group" aria-label="Draggable interface status panel">
        <span className="drag-handle">DRAG</span><small>INTERFACE STATUS</small><strong><i /> ONLINE</strong><em>{dragging ? "PANEL MOVING" : "REACT INTERACTION READY"}</em>
      </div>
    </div>
    ...
    <div className="radar-interaction-state" style={{ ["--radar-velocity" as string]: radarVelocity }} aria-hidden="true" />
  </>;
}
